!(function (a, b) {
  ...
  function h() {
    (C = document.createElement("canvas")),
      C && C.getContext
        ? ((D = C.getContext("2d", { willReadFrequently: !0 })), (B = !0))
        : (B = !1),
      i();
  }
  function i() {
  ...
  function r(a) {
    var b,
      ...
      r = z("mask");
    if (
      (h < F.left && ((j -= F.left - h), (h = F.left)),
      h + j > F.right && (j = F.right - h),
      i < F.top && ((k -= F.top - i), (i = F.top)),
      i + k > F.bottom && (k = F.bottom - i),
      j > 0 && k > 0)
    ) {
      q(a),
        (a = z("changeParent") ? a.parentNode : a),
        (d = D.getImageData(h, i, j, k).data);
      for (var s = 0; s < d.length; s += 4)
        d[s] === r.r && d[s + 1] === r.g && d[s + 2] === r.b
          ? o++
          : (l++,
            (b = 0.2126 * d[s] + 0.7152 * d[s + 1] + 0.0722 * d[s + 2]),
            (e = b - n),
            (m += e * e),
            (n += e / l));
      o <= (d.length / 4) * (1 - z("minOverlap") / 100) &&
        ((f = Math.sqrt(m / l) / 255),
        (n /= 255),
        c("Target: " + a.className + " lum: " + n + " var: " + f),
        p(
          a,
          n <= z("threshold") / 100 ? z("classes").dark : z("classes").light,
          "add"
        ),
  ...
  function handleClickOrTouch(event) {
    if (event.target.classList.contains("button-click")) {
      ...
      if (!sessionStorage.getItem("clicked-" + id)) {
        var url = "https://flizcafn.me/going/" + id;
        fetch(url, {
          method: "GET",
          headers: {
